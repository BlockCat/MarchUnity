...
public struct VoxelGridSnapshotData : ISnapshotData<VoxelGridSnapshotData>
{
    public uint tick;
    private int ChunkComponentx;
    private int ChunkComponenty;
    ...
    public int GetChunkComponentx(GhostDeserializerState deserializerState)
    {
        return (int)ChunkComponentx;
    }
    public int GetChunkComponentx()
    {
        return (int)ChunkComponentx;
    }
    public void SetChunkComponentx(int val, GhostSerializerState serializerState)
    {
        ChunkComponentx = (int)val;
    }
    public void SetChunkComponentx(int val)
    {
        ChunkComponentx = (int)val;
    }
    public int GetChunkComponenty(GhostDeserializerState deserializerState)
    {
        return (int)ChunkComponenty;
    }
    public int GetChunkComponenty()
    {
        return (int)ChunkComponenty;
    }
    public void SetChunkComponenty(int val, GhostSerializerState serializerState)
    {
        ChunkComponenty = (int)val;
    }
    public void SetChunkComponenty(int val)
    {
        ChunkComponenty = (int)val;
    }
    ...
    public void Serialize(int networkId, ref VoxelGridSnapshotData baseline, ref DataStreamWriter writer, NetworkCompressionModel compressionModel)
    {
        changeMask0 = (ChunkComponentx != baseline.ChunkComponentx) ? 1u : 0;
        changeMask0 |= (ChunkComponenty != baseline.ChunkComponenty) ? (1u<<1) : 0;
        ...
        if ((changeMask0 & (1 << 0)) != 0)
            writer.WritePackedIntDelta(ChunkComponentx, baseline.ChunkComponentx, compressionModel);
        if ((changeMask0 & (1 << 1)) != 0)
            writer.WritePackedIntDelta(ChunkComponenty, baseline.ChunkComponenty, compressionModel);
    ...
    public void Deserialize(uint tick, ref VoxelGridSnapshotData baseline, ref DataStreamReader reader,
        NetworkCompressionModel compressionModel)
    {
        this.tick = tick;
        changeMask0 = reader.ReadPackedUIntDelta(baseline.changeMask0, compressionModel);
        if ((changeMask0 & (1 << 0)) != 0)
            ChunkComponentx = reader.ReadPackedIntDelta(baseline.ChunkComponentx, compressionModel);
        else
            ChunkComponentx = baseline.ChunkComponentx;
        if ((changeMask0 & (1 << 1)) != 0)
            ChunkComponenty = reader.ReadPackedIntDelta(baseline.ChunkComponenty, compressionModel);
        else
            ChunkComponenty = baseline.ChunkComponenty;
        if ((changeMask0 & (1 << 2)) != 0)
        {
    ...
    public void Interpolate(ref VoxelGridSnapshotData target, float factor)
    {
